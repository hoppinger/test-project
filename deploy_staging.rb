...
git_url = 'git@github.com:hoppinger/test-project.git'
...
on cluster_nodes, in: :parallel do |host|
  ...
  within project_dir do
    ...
    execute :cp, 'appsettings.json', 'repo/appsettings.staging.json'
...
on cluster_nodes.first, in: :parallel do |host|
  execute :docker, "run -t #{image_tag} dotnet ef database update -c WatchtowerContext"
end
