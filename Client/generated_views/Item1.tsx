...
export function Item1_Item1_Group1_can_create(self:Item1Context) {
  let state = self.state()
  return state.Group1 == "loading" ? false : state.Group1.CanCreate
}
export function Item1_Item1_Group1_can_delete(self:Item1Context) {
  let state = self.state()
  return state.Group1 == "loading" ? false : state.Group1.CanDelete
}
export function Item1_Item1_Group1_page_index(self:Item1Context) {
  let state = self.state()
  return state.Group1 == "loading" ? 0 : state.Group1.PageIndex
}
export function Item1_Item1_Group1_page_size(self:Item1Context) {
  let state = self.state()
  return state.Group1 == "loading" ? 25 : state.Group1.PageSize
}
export function Item1_Item1_Group1_search_query(self:Item1Context) {
  let state = self.state()
  return state.Group1 == "loading" ? null : state.Group1.SearchQuery
}
export function Item1_Item1_Group1_num_pages(self:Item1Context) {
  let state = self.state()
  return state.Group1 == "loading" ? 1 : state.Group1.NumPages
}

export function load_relation_Item1_Item1_Group1(self:Item1Context, force_first_page:boolean, callback?:()=>void) {
  let state = self.state()
  ...
  Permissions.can_view_Group1() ?
    prelude(() =>
      Api.get_Item1_Item1_Group1S(self.props.entity, Item1_Item1_Group1_page_index(self), Item1_Item1_Group1_page_size(self), Item1_Item1_Group1_search_query(self)).then(Group1S =>
...
export function load_relations_Item1(self, callback?:()=>void) {
  load_relation_Item1_Item1_Group1(self, false, 
        () => callback && callback())
}
...
export function render_local_menu_Item1(self:Item1Context) {
  ...
  return <div className="local-menu">
          <div className="local_menu_entries">
            ...
            {!Permissions.can_view_Group1() ? null :
                  <div key={"Item1_Group1"} className={`local_menu_entry${self.props.shown_relation == "Item1_Group1" ? " local_menu_entry--active" : ""}`}>
                    <a onClick={() =>
                      load_relation_Item1_Item1_Group1(self,
                        false,
                        
                        () => self.props.set_shown_relation("Item1_Group1"))
                    }>
                      {i18next.t('Item1_Group1S_inverted')}
                    </a>
                  </div>
                }  
          </div>
...
export function render_Item1_Item1_Group1(self:Item1Context, context:"presentation_structure"|"default") {
  if ((context == "default" && self.props.shown_relation != "all" && self.props.shown_relation != "Item1_Group1") || !Permissions.can_view_Group1())
    return null
  let state = self.state()
  return <div>
    
    { List.render_relation("item1_item1_group1",
   "Item1",
   "Group1",
   "Group1S",
  ...
  (
      state.Group1 != "loading" ?
        ...
        state.Group1,
      Item1_Item1_Group1_page_index(self),
      Item1_Item1_Group1_num_pages(self),
      new_page_index => {
          let state = self.state()
          state.Group1 != "loading" &&
            ...
            <div key={i_id}>
              {
                Group1Views.Group1({
                  ...
                  unlink: !Permissions.can_delete_Group1_Item1() ?
                    null
                    :
                    () => confirm(i18next.t('Are you sure?')) && Api.unlink_Group1_Item1_Group1S(i.element, self.props.entity).then(() =>
                      load_relation_Item1_Item1_Group1(self, false, ))
                })
              }
            </div>
      ...
      () =>
        <div>
          {Permissions.can_create_Group1() && Permissions.can_create_Group1_Item1() && Item1_Item1_Group1_can_create(self) ? render_new_Item1_Item1_Group1(self) : null}
          {Permissions.can_create_Group1_Item1() ? render_add_existing_Item1_Item1_Group1(self) : null}
        </div>)
    }
...
export function render_relations_Item1(self:Item1Context) {
  return <div className="relations">
      { render_Item1_Item1_Group1(self, "default") }
      
    </div>
}

export function render_add_existing_Item1_Item1_Group1(self:Item1Context) {
      ...
      <div className="button__actions">
        {
          ...
          React.createElement(List.AddToRelation,
            {
              relation_name:"item1_item1_group1",
              source_name:"Item1",
              target_name:"Group1",
                  ...
                  <a className="group__button button button--existing"
                    onClick={() =>
                        self.setState({...self.state(), add_step_Group1:"saving"}, () =>
                          Api.link_Item1_Item1_Group1S(self.props.entity, i).then(() =>
              ...
              get_items:[
                { name: "Group1", get: async(i,s) => Api.get_unlinked_Item1_Item1_Group1S(self.props.entity, i, s) },
              ]
            })
        }
      </div>
...
export function render_new_Item1_Item1_Group1(self:Item1Context) {
    let state = self.state()
    return  self.props.mode == "edit" ?
      <div className="button__actions">
        <div className="new-group1">
              <button disabled={state.Group1 == "loading" ? true : state.Group1.TotalCount >= 1} 
                      ...
                      onClick={() =>
                          Api.create_linked_Item1_Item1_Group1S_Group1(self.props.entity).then(e => {
                              e.length > 0 &&
                              Api.update_Group1(
                                ...
                                load_relation_Item1_Item1_Group1(self, true, () =>
                                    self.setState({...self.state(), add_step_Group1:"closed"})
                                  )
                                )
                          })
