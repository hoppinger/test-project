...
export function Item2A_Item2A_Group2_can_create(self:Item2AContext) {
  let state = self.state()
  return state.Group2 == "loading" ? false : state.Group2.CanCreate
}
export function Item2A_Item2A_Group2_can_delete(self:Item2AContext) {
  let state = self.state()
  return state.Group2 == "loading" ? false : state.Group2.CanDelete
}
export function Item2A_Item2A_Group2_page_index(self:Item2AContext) {
  let state = self.state()
  return state.Group2 == "loading" ? 0 : state.Group2.PageIndex
}
export function Item2A_Item2A_Group2_page_size(self:Item2AContext) {
  let state = self.state()
  return state.Group2 == "loading" ? 25 : state.Group2.PageSize
}
export function Item2A_Item2A_Group2_search_query(self:Item2AContext) {
  let state = self.state()
  return state.Group2 == "loading" ? null : state.Group2.SearchQuery
}
export function Item2A_Item2A_Group2_num_pages(self:Item2AContext) {
  let state = self.state()
  return state.Group2 == "loading" ? 1 : state.Group2.NumPages
}

export function load_relation_Item2A_Item2A_Group2(self:Item2AContext, force_first_page:boolean, callback?:()=>void) {
  let state = self.state()
  ...
  Permissions.can_view_Group2() ?
    prelude(() =>
      Api.get_Item2A_Item2A_Group2S(self.props.entity, Item2A_Item2A_Group2_page_index(self), Item2A_Item2A_Group2_page_size(self), Item2A_Item2A_Group2_search_query(self)).then(Group2S =>
...
export function load_relations_Item2A(self, callback?:()=>void) {
  load_relation_Item2A_Item2A_Group2(self, false, 
        () => callback && callback())
}
...
export function render_Item2A_Item2A_Group2(self:Item2AContext, context:"presentation_structure"|"default") {
  if ((context == "default" && self.props.shown_relation != "all" && self.props.shown_relation != "Item2A_Group2") || !Permissions.can_view_Group2())
    return null
  let state = self.state()
  return <div>
    
    { List.render_relation("item2a_item2a_group2",
   "Item2A",
   "Group2",
   "Group2S",
  ...
  (
      state.Group2 != "loading" ?
        ...
        state.Group2,
      Item2A_Item2A_Group2_page_index(self),
      Item2A_Item2A_Group2_num_pages(self),
      new_page_index => {
          let state = self.state()
          state.Group2 != "loading" &&
            ...
            <div key={i_id}>
              {
                Group2Views.Group2({
                  ...
                  unlink: !Permissions.can_delete_Group2_Item2A() ?
                    null
                    :
                    () => confirm(i18next.t('Are you sure?')) && Api.unlink_Group2_Item2A_Group2S(i.element, self.props.entity).then(() =>
                      load_relation_Item2A_Item2A_Group2(self, false, ))
                })
              }
            </div>
      ...
      () =>
        <div>
          {Permissions.can_create_Group2() && Permissions.can_create_Group2_Item2A() && Item2A_Item2A_Group2_can_create(self) ? render_new_Item2A_Item2A_Group2(self) : null}
          {Permissions.can_create_Group2_Item2A() ? render_add_existing_Item2A_Item2A_Group2(self) : null}
        </div>)
    }
...
export function render_add_existing_Item2A_Item2A_Group2(self:Item2AContext) {
      ...
      <div className="button__actions">
        {
          ...
          React.createElement(List.AddToRelation,
            {
              relation_name:"item2a_item2a_group2",
              source_name:"Item2A",
              target_name:"Group2",
                  ...
                  <a className="group__button button button--existing"
                    onClick={() =>
                        self.setState({...self.state(), add_step_Group2:"saving"}, () =>
                          Api.link_Item2A_Item2A_Group2S(self.props.entity, i).then(() =>
              ...
              get_items:[
                { name: "Group2", get: async(i,s) => Api.get_unlinked_Item2A_Item2A_Group2S(self.props.entity, i, s) },
              ]
            })
        }
      </div>
...
export function render_new_Item2A_Item2A_Group2(self:Item2AContext) {
    let state = self.state()
    return  self.props.mode == "edit" ?
      <div className="button__actions">
        <div className="new-group2">
              <button disabled={state.Group2 == "loading" ? true : state.Group2.TotalCount >= 4} 
                      ...
                      onClick={() =>
                          Api.create_linked_Item2A_Item2A_Group2S_Group2(self.props.entity).then(e => {
                              e.length > 0 &&
                              Api.update_Group2(
                                ({ ...e[0], Name:"" } as Models.Group2)).then(() =>
                                load_relation_Item2A_Item2A_Group2(self, true, () =>
                                    self.setState({...self.state(), add_step_Group2:"closed"})
                                  )
                                )
                          })
